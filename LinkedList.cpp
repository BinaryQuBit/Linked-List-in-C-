...
void LinkedList::deleteItem (int item)
{
   ...
   else
   {
      ...
      while (currPtr->next != NULL && currPtr->next->datum != item) // Added another condition where currPtr->next cant be equal to NULL
      {
	      currPtr = currPtr->next;
      }

      if(currPtr->next != NULL) //added if statment so it has binary neuro to either delete the item in the middle of the list or output else statement
      {
        //Save its location
        delPtr = currPtr->next;
      ...
      else //else statement to print if the value of the item doesnt match the input, so no core dump
      {
